package ru.syn.quotes.controllers;

import com.pengrad.telegrambot.TelegramBot;
import com.pengrad.telegrambot.UpdatesListener;
import com.pengrad.telegrambot.model.Update;
import com.pengrad.telegrambot.request.SendMessage;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import ru.syn.quotes.models.Chat;
import ru.syn.quotes.models.Quote;
import ru.syn.quotes.repositories.ChatRepository;
import ru.syn.quotes.services.QuoteService;

@Service
public class BotController {

    @Autowired
    ChatRepository chatRepository;

    @Autowired
    QuoteService service;

    private final TelegramBot bot;

    public BotController() {
        String BOT_TOKEN = "6981558699:AAHwrBHhPgGpkVf8RiCd6LzoK2Ss-XSWo20";
        bot = new TelegramBot(BOT_TOKEN);

        bot.setUpdatesListener(updates -> {

            for (Update update : updates) {
                handleUpdate(update);
            }

            return UpdatesListener.CONFIRMED_UPDATES_ALL;
        });

    }

    private void handleUpdate(Update update) {
        String text = update.message().text();
        long chatId = update.message().chat().id();
        var rawChat = chatRepository.findByChatldEquals(chatId);
        Chat chat;
        if (rawChat.isPresent()) {
            chat = rawChat.get();
        } else {
            var _chat = new Chat();
            _chat.setChatld(chatId);
            _chat.setLastld(0);
            chat = chatRepository.save(_chat);
        }
        switch (text) {
            case "/start":
            case "/next":
                sendNextQuote(chat);
                break;
            case "/prev":
                sendPrevQuote(chat);
                break;
            case "/rand":
                sendRandom(chat);
                break;
        }
    }

    private void sendNextQuote(Chat chat) {
        Quote quote = null;
        int newId = chat.getLastld();
        while (quote == null) {
            newId++;
            quote = service.getById(newId);
        }
        chat.setLastld(quote.getQuoteId());
        chatRepository.save(chat);
        sendText(chat.getChatld(), quote.getText());
    }

    private void sendPrevQuote(Chat chat) {
        Quote quote = null;
        int newId = chat.getLastld();
        while (quote == null) {
            newId--;
            if(newId<2) newId=2;
            quote = service.getById(newId);
        }
        chat.setLastld(quote.getQuoteId());
        chatRepository.save(chat);
        sendText(chat.getChatld(), quote.getText());
    }

    private void sendRandom(Chat chat) {
        Quote quote = service.getRandom();
        sendText(chat.getChatld(), quote.getText());
    }

    private  void sendText(long chatId, String text){
        bot.execute(new SendMessage(chatId, text));
    }
}